<x-app-layout>
    <div class="max-w-4xl mx-auto p-6">
        ...
        @if($documents->isEmpty())
            <p class="text-gray-500 text-sm">Aucun document pour cette séance.</p>
        @else
            <div class="bg-white shadow rounded">
                <ul class="divide-y">
                    @foreach($documents as $doc)
                        @php
                            $rowClass = match($doc->status) {
                        ...
                        <li>
                            <div class="flex items-center justify-between px-4 py-3 {{ $rowClass }}">
                                ...
                                <div class="flex items-center gap-3">
                                    ...
                                    @if($doc->status === 'failed')
                                        <form method="POST" action="{{ route('admin.documents.reindex', $doc) }}">
                                            @csrf
                                            <button type="submit"
                            ...
                            @if($doc->type === 'deliberation' && $doc->annexes->isNotEmpty())
                                <details class="border-t border-gray-100 group/annexe">
                                    <summary class="flex items-center gap-1.5 px-4 py-2 text-xs font-medium text-indigo-600 cursor-pointer hover:bg-indigo-50 transition list-none select-none">
                                        ...
                                        {{ $doc->annexes->count() }} annexe{{ $doc->annexes->count() > 1 ? 's' : '' }}
                                    </summary>
